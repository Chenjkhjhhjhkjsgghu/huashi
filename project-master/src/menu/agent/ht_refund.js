...
class App extends React.Component {
  ...
  Submit()
  {
    ...
    axios.post('/ht_refund',this.edu).then(res=>{
          message.info('还款成功')
        this.edu.debt_after= res.debt_after/100
  ...
  render() {
    return (
     ...
     <h3 style={{ textAlign:'left', fontSize: '22px', lineHeight: '50px' }}><span style={{ color: 'rgb(64, 169, 255)' }}>后台还款</span></h3>
      <div className='dialog'  style={this.state.display}>
            <div>
            <h2><Icon type="check" style={{color:'green'}}/>还款成功</h2>
            <span>操作id:<b>{this.state.edu.id}</b></span>
            <span>操作金额:<b style={{color:'red'}}>{this.state.edu.amount}</b></span>
            <span>操作前欠款:<b>{this.state.edu.debt}</b></span>
            <span>操作后欠款:<b>{this.state.edu.debt_after}</b></span>
     ...
     <div><Input addonBefore='还款金额:' defaultValue={this.state.edu.amount} onChange={this.change_edu.bind(this)}/></div>
