...
class App extends React.Component{
   ...
   fn(params={}){
    ...
    axios.get('/get_to_audit_score_exchange',params).then(res=>{
        let authority = {sh:false,delete:false};
        if(Object.keys(res.authority.cancel_audit_score_exchange).length>0)
        {
            authority.sh = true;
        }
        if(Object.keys(res.authority.pass_audit_score_exchange).length>0)
        {
        ...
        res.output.map((i,index)=>{
           ...
           i.todo = (
               <div>
                   {authority.sh?
                   <Popconfirm title={`确定让ID${i.user_id}通过审核吗?`} onConfirm={this.confirm.bind(this)} onCancel={this.cancel.bind(this)} okText="是" cancelText="否">
                   <Button style={{marginLeft:'5px'}} onClick={this.setId.bind(this)} name={i.order_id}>通过</Button>
                   </Popconfirm>:false}
                  {
                      authority.delete? <Popconfirm title={`确定要取消ID${i.user_id}的审核吗?`} onConfirm={this.confirms.bind(this)} onCancel={this.cancel.bind(this)} okText="是" cancelText="否">
                      <Button style={{marginLeft:'5px'}} onClick={this.setId.bind(this)} name={i.order_id}>取消</Button>
                      </Popconfirm>:false
                  }
               </div>
           )
